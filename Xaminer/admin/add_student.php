...
<body>
	...
	<div class="container">
		<div class="jumbotron">
			<h2 class="text-center">Enter the Student Details and add to Database.</h2>
		</div>
		<form method="post" action="add_student_to_db.php">
			...
			<div style="border: 1px solid black; border-radius: 5px; padding:10px; margin-bottom: 10px;">
		  	...
		  	<h3>Subjects Enrolled in: </h3>
		  	<div style="border: 1px solid black; border-radius: 5px; padding:10px">
			  <div class="form-row">
			    <div class="form-group col-md-6">
			    	<label>Choose Subject 1: </label>
			    	<?php
			    		echo"<select name='s1' class='custom-select mb-3'>";
			    ...
			    <div class="form-group col-md-6">
			    	<label>Choose Subject 2: </label>
			    	<?php
			    		echo"<select name='s2' class='custom-select mb-3'>";
			    		$query = "SELECT * FROM course";
			    		$res = mysqli_query($con,$query) or die(msyqli_error($con));
			    		while($row = mysqli_fetch_array($res)){
			    			echo"<option value='".$row['id']."'>".$row['subject_name']."</option>";
			    ...
			    <div class="form-group col-md-6">
			      	<label>Choose Subject 3: </label>
			    	<?php
			    		echo"<select name='s3' class='custom-select mb-3'>";
			    		$query = "SELECT * FROM course";
			    		$res = mysqli_query($con,$query) or die(msyqli_error($con));
			    		while($row = mysqli_fetch_array($res)){
			    			echo"<option value='".$row['id']."'>".$row['subject_name']."</option>";
			    ...
			    <div class="form-group col-md-6">
			      	<label>Choose Subject 4: </label>
			    	<?php
			    		echo"<select name='s4' class='custom-select mb-3'>";
			    		$query = "SELECT * FROM course";
			    		$res = mysqli_query($con,$query) or die(msyqli_error($con));
			    		while($row = mysqli_fetch_array($res)){
			    			echo"<option value='".$row['id']."'>".$row['subject_name']."</option>";
			...
			<br>
			<button class="btn btn-warning btn-lg btn-block" type="submit">Add Student</button>
		</form>
